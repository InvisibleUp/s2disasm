...
The Dynamic_Normal (or zone=specific dynamic subroutine) uses these scripts to
reload certain tiles, thus animating them. All the relevant art must be
uncompressed, because otherwise the subroutine would spend so much time waiting
for the art to be decompressed that the VBLANK window would close before all
the animating was done.
